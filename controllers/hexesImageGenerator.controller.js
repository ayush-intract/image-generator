    ...
    async generatorHexesImage(req, res) {
        try {
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                //     "cardData": data[i].cardData,
                //     "cardTheme": data[i].theme || "superchain",
                // }
                // Push the promise to the array
                promises.push(hexesImageGenerationService.generateHexesService(data[i]).then(r => {
                    // return r;
                    return { cardType: data[i].cardType, publicUrl:  r?.publicUrl?.replace('%2F', '/')?.replace('storage.googleapis.com/static.highongrowth.xyz', 'static.highongrowth.xyz'), profileImageLink:  r.profileImageLink?.replace('%2F', '/').replace('storage.googleapis.com/static.highongrowth.xyz', 'static.highongrowth.xyz') };
                }));
            }
